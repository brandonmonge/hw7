# Your name: Brandon Monge
# Your email: brandom@umich.edu
...
def make_players_table(data, cur, conn):
    ...
    cur.execute(""" CREATE TABLE IF NOT EXISTS Players 
                 (id INTEGER PRIMARY KEY, name TEXT, position_id INTEGER,
                 birthyear INTEGER, nationality TEXT, 
                 FOREIGN KEY (position_id) REFERENCES Positions (id) ) """)
    
    for p in data['squad']:
        cur.execute("SELECT id FROM Positions WHERE position = ?", (p['position'],))
        pos_id = cur.fetchone()[0]
        cur.execute( """ INSERT OR IGNORE INTO Players 
                         (id, name, position_id, birthyear, nationality) VALUES (?,?,?,?,?)""", 
                         (p['id'], p['name'], pos_id, int(p['dateOfBirth'][:4]), p['nationality']) )
...
def nationality_search(countries, cur, conn):
    questions = ','.join( ['?'] * len(countries) )
    cur.execute(f"SELECT name, position_id, nationality FROM Players WHERE nationality IN ({questions})", countries)
    return cur.fetchall()
...
def position_birth_search(position, age, cur, conn):
    cur.execute("SELECT P.name, Pos.position, P.birthyear FROM Players P JOIN Positions Pos ON P.position_id = Pos.id WHERE Pos.position = ? AND P.birthyear > ?", 
                ( position, 2023 - age ) )
    return cur.fetchall()
